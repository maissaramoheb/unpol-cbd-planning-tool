...
import { MissionExplorerCompare } from './MissionExplorerCompare';
import { Button } from '../ui/Button';
import { AlertTriangle, Globe, List, X } from 'lucide-react';
import { filterMissionExplorerEntries } from '../lib/explorerFilters';
import { hasMeaningfulWork, resolvePlanningContext } from '../lib/planningContext';
...
export const MissionExplorer: React.FC<MissionExplorerProps> = ({
  ...
  // Comparison State (local React state only; never persisted into project data)
  ...
  const [selectedVerification, setSelectedVerification] = useState<
    'all' | 'current-reference' | 'review-required' | 'training-only'
  >('all');
  ...
  const filteredEntries = useMemo(
    ...
    [searchQuery, selectedRegion, selectedType, selectedStatus, showFictional, selectedVerification]
  );
  ...
  const handleToggleCompare = (id: string) => {
    if (compareIds.includes(id)) {
      ...
      if (compareIds.length < 3) {
        ...
        setCompareWarning('Compare supports up to 3 contexts.');
      }
    }
  };
  ...
  // Resolved canonical contexts for comparison
  const comparedContexts = useMemo(() => {
    return compareIds
      .map((id) => resolvePlanningContext(id))
      .filter((c): c is PlanningContext => c !== null);
  }, [compareIds]);
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
      ...
      >
        ...
        {/* View Switcher: Dedicated Comparison View vs Explorer Search/Browse */}
        {isComparing ? (
          <div className="flex-1 overflow-hidden flex flex-col">
            <MissionExplorerCompare
              contexts={comparedContexts}
                ...
                <div className="flex-1 flex flex-col gap-4">
                  ...
                  <div className={`${viewMode === 'list' ? 'block' : 'hidden'} lg:block flex-1`}>
                    <MissionExplorerList
                      ...
                      onVerificationChange={(value) => handleFilterChange(() => setSelectedVerification(value))}
                      compareIds={compareIds}
              ...
              >
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  <div
                    ...
                    role="list"
                    aria-label="Selected contexts for comparison"
                  >
                    {compareIds.map((id) => {
                      const entry = defaultExplorerSeeds.find((e) => e.id === id);
                      const acronym = entry?.missionAcronym || id;
                      return (
                        <span
                          key={id}
                          role="listitem"
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button
                    ...
                    className={`text-xs font-bold ${compareIds.length < 2 ? 'opacity-50 cursor-not-allowed' : ''}`}
                  >
