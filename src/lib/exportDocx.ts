...
const text = (value: string, options: { bold?: boolean; color?: string; size?: number; italics?: boolean } = {}) =>
  new TextRun({ text: value || 'Not recorded', ...options });
...
const heading = (value: string, level: typeof HeadingLevel.HEADING_1 | typeof HeadingLevel.HEADING_2 | typeof HeadingLevel.HEADING_3 = HeadingLevel.HEADING_1) =>
  new Paragraph({ text: value, heading: level, spacing: { before: level === HeadingLevel.HEADING_1 ? 160 : 100, after: 100 }, keepNext: true });
...
const infoTable = (items: Array<[string, string]>) => new Table({
  ...
  rows: Array.from({ length: Math.ceil(items.length / 2) }, (_, rowIndex) => new TableRow({
    ...
    children: [0, 1].map(columnIndex => {
      ...
      return item ? cell(item[0], item[1]) : cell('', '', WHITE);
    })
  }))
});
...
export async function createPlanningBriefDocx(model: PlanningBriefModel): Promise<Blob> {
  ...
  const children = [
    ...
    pageBreak(), heading('2 · Context & Key Evidence'),
    ...
    pageBreak(), heading('3 · Stakeholder Landscape'),
    ...
    ...model.priorities.flatMap((priority, index) => [
      ...(index === 0 || index === 2 ? [pageBreak(), heading(index === 0 ? '4 · CBD Priorities' : '5 · CBD Priorities (continued)')] : []),
      priorityTable(priority),
      paragraph('')
    ]),

    pageBreak(), heading('6 · Sequencing & Implementation Pathway'),
    ...
    pageBreak(), heading('7 · Monitoring, Evidence Gaps & Planning Controls'),
    ...
    heading('Quality-Control Flags', HeadingLevel.HEADING_2), ...bulletList(model.warnings.slice(0, 6).map(item => item.message), 'No active quality-control warnings.'),
    heading('Limitations / Advisory Note', HeadingLevel.HEADING_2), paragraph(ADVISORY_NOTE),

    pageBreak(), heading('Annex A · Full PESTEL-S Analysis'),
    ...Object.values(data.pestels).flatMap(item => [heading(item.name, HeadingLevel.HEADING_2), paragraph(item.finding), paragraph(`Why: ${item.why}`), paragraph(`Sequencing: ${item.sequencing}`)]),
    pageBreak(), heading('Annex B · Stakeholder Register'),
    infoTable(data.stakeholders.flatMap(item => [[item.name, `${item.category} · Influence ${item.influence} · Position ${item.position} · Relevance ${item.relevance}\n${item.engagement}`] as [string, string]])),
    pageBreak(), heading('Annex C · Evidence & Source Register'),
    ...model.evidence.flatMap(item => [heading(`${item.id} · ${item.title}`, HeadingLevel.HEADING_3), paragraph(`${item.type} · Confidence ${item.confidence}/5 · Reviewed ${item.date}`), paragraph(`Linked to: ${item.attachedTo}`), paragraph(item.comment, { italics: true })]),
    pageBreak(), heading('Annex D · Full Quality-Control Register'), ...bulletList(model.warnings.map(item => item.message), 'No active quality-control warnings.')
  ];
  ...
  const document = new Document({
    ...
    styles: {
      ...
      paragraphStyles: [
        { id: 'Heading1', name: 'Heading 1', basedOn: 'Normal', next: 'Normal', quickFormat: true, run: { font: 'Arial', size: 28, bold: true, color: BLUE }, paragraph: { spacing: { before: 180, after: 100 }, keepNext: true } },
        { id: 'Heading2', name: 'Heading 2', basedOn: 'Normal', next: 'Normal', quickFormat: true, run: { font: 'Arial', size: 22, bold: true, color: BLUE }, paragraph: { spacing: { before: 130, after: 70 }, keepNext: true } },
        { id: 'Heading3', name: 'Heading 3', basedOn: 'Normal', next: 'Normal', quickFormat: true, run: { font: 'Arial', size: 20, bold: true, color: SLATE }, paragraph: { spacing: { before: 100, after: 50 }, keepNext: true } }
      ]
    },
    sections: [{
      properties: { page: { size: { width: 11906, height: 16838 }, margin: { top: 1134, right: 1134, bottom: 1134, left: 1134, header: 600, footer: 600 } } },
      headers: { default: new Header({ children: [new Paragraph({ border: { bottom: { style: BorderStyle.SINGLE, color: 'CBD5E1', size: 4 } }, children: [text('UNPOL CBD PLANNING BRIEF', { bold: true, color: BLUE, size: 16 }), text(`   |   ${model.status}`, { color: SLATE, size: 16 })] })] }) },
      footers: { default: new Footer({ children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [text(`${p.countryName || 'Planning context'}   ·   Page `, { color: SLATE, size: 16 }), new TextRun({ children: [PageNumber.CURRENT], color: SLATE, size: 16 })] })] }) },
      children
    }]
  });
